...
const ReflectiveCard: React.FC<ReflectiveCardProps> = ({
    ...
    const startWebcam = async () => {
      try {
        ...
        toast({
          ...
          description: 'Please enable camera permissions to see the reflective effect.'
        });
      }
    };
  ...
  return (
    ...
    >
      ...
      {!hasCameraPermission && (
         <div className="absolute inset-0 z-[5] bg-black/50 flex flex-col items-center justify-center text-center p-4">
            <VideoOff size={48} className="text-white/50 mb-4"/>
            <Alert variant="destructive" className="bg-destructive/50 border-destructive">
              <AlertTitle>Camera Access Denied</AlertTitle>
              <AlertDescription>
                Enable camera permissions to see the live reflective background.
              </AlertDescription>
            </Alert>
         </div>
      )}
      ...
      <div className="relative z-10 h-full flex flex-col justify-between p-8 text-[var(--text-color,white)] bg-[var(--overlay-color,rgba(255,255,255,0.05))]">
        ...
        <div className="flex-1 flex flex-col justify-end items-center text-center gap-6 mb-8">
          <div className="text-center">
            <h2 className="text-2xl font-bold tracking-[0.05em] m-0 mb-2 drop-shadow-md">{user.name?.toUpperCase()}</h2>
            <p className="text-xs tracking-[0.2em] opacity-70 m-0 uppercase">{roleDisplayName(user.role)}</p>
            <p className="text-xs tracking-[0.1em] opacity-50 m-0 mt-1">@{user.username}</p>
          </div>
        </div>
        ...
        <div className="flex justify-between items-end border-t border-white/20 pt-6">
          <div className="flex flex-col gap-1">
            ...
            <span className="font-mono text-sm tracking-[0.05em]">{String(user.id)?.substring(0, 14)}</span>
          </div>
